...
public class UsefulDresser {
	...
	/**
     * The method compareTo compares the total drawers from two objects dressers.
     * This is done by subtracting the total drawers of the second object dresser from the
     * first object dresser.  An integer value is returned, either (-), (0), or (+).
     * The (-) value indicates that the first dresser is smaller than the second.  The (0)
     * value indicates the dressers are equal in size.  The (+) value indicates that the first
     * dresser is bigger than the second.
     * */
	public int compareTo(UsefulDresser usefuldresser) {
        return this.totalDrawers - usefuldresser.totalDrawers;
    }

    /**
     * The method fullerThan compares the ratios of used drawers to empty drawers in two object dressers.
     * The comparison is made using cross multiplication.  The ratio of used (numerator) to empty
     * (denominator) drawers of the first object is cross multiplied with the ratio of used to empty
     * drawers in the second object.  If the first dresser has a smaller ratio of used to empty drawers than
     * the second dresser, solution = -1.  If the ratios are equal solution = 0.  If the first
     * dresser has a greater ratio of used to empty drawers than the second dresser, solution = 1.
     * 
     * Note: the use of cross multiplication to compare the ratios of used to empty drawers only works
     * when inputs are positive.
     * */
	public double fullerThan(UsefulDresser usefuldresser) {
        double solution;
		if (this.usedDrawers * (usefuldresser.totalDrawers - usefuldresser.usedDrawers) 
        < (this.totalDrawers - this.usedDrawers) * usefuldresser.usedDrawers) {
			solution = -1;
		}
		else if (this.usedDrawers * (usefuldresser.totalDrawers - usefuldresser.usedDrawers) 
        == (this.totalDrawers - this.usedDrawers) * usefuldresser.usedDrawers) {
			solution = 0;
		}
		else {
